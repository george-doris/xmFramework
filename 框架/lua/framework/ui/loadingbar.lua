...
function UI.LoadingBar:ctor(name, parent)
    ...
    self._clipFrame = UI.ClipFrame.new("_CLIPFRAME",self)
    UI.Inner(self._clipFrame)
    self._clipFrame:setAnchorType(UI.AnchorType.LEFT_BOTTOM)
    self._clipFrame:setPosition(0,0)
    self._clipFrame:setVisible(false)
    self._progressBar = UI.Backdrop.new("_PROGRESSBAR",self._clipFrame)
