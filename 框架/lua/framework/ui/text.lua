...
local function _updateShadow(self)
    ...
    if self._shadowEnabled then
        ...
        local offset = self:getParentOffset()
        japi.DzFrameSetPoint(self._shadow, self._anchorType, self:getParentFrameID(),
                UI.AnchorType.LEFT_BOTTOM, self._x+self._shadowOffset.x+offset.x, self._y+self._shadowOffset.y+offset.y)


        japi.DzFrameSetText(self._shadow, UI.SetTextColor(UI.ClearTextColor(self._text),self._shadowColor.r,self._shadowColor.g,self._shadowColor.b,self._shadowColor.a))
...
function UI.Text.GetStringExtent(str, font, h, falg)
    ...
    japi.DzFrameSetFont(_font_size_com, font, h, falg)
    ...
    for index, line in ipairs(t) do
        for index1, value in ipairs(line) do
            ...
            line_width = line_width + japi.FrameGetTextWidth(_font_size_com)
        end
        if line_width > 0 then
            if line_width > width then width = line_width end
            height = height + japi.FrameGetTextHeight(_font_size_com)
            line_width = 0
        end
    end
...
function UI.Text:AutoSize(max_width)
    if max_width == nil then
        ...
        local width, height = UI.Text.GetStringExtent(self:getText(),
            self:getFont(), self:getFontSize(), self:getFontStyle())
